...
## Automation Rules
- **Build Time Ads:** `TravianTaskQueue` automatically clicks the "Watch Video" button in the building list on `dorf1/dorf2`. `TravianAdSkipper` handles the confirmation dialog, skips the video, and claims the reward.
- **Coordination:** Both scripts must be active for full build-time reduction automation.
